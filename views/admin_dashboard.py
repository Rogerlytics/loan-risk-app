...
# -------------------------------------------------------------------
# Realtime subscription (runs in background thread)
# -------------------------------------------------------------------
def _start_realtime_listener(supabase_client):
    """Subscribe to messages table changes and notify the main thread."""
    if "realtime_initialised" in st.session_state:
        return  # already running

    def handle_realtime_change(payload):
        # Payload example: {"type": "INSERT", "table": "messages", "record": {...}}
        st.session_state.new_realtime_event = True
        st.session_state.latest_payload = payload

    # Create a realtime channel for the 'messages' table
    try:
        channel = supabase_client.realtime.channel("public:messages")
        channel.on("*", handle_realtime_change).subscribe()
        st.session_state.realtime_initialised = True
        st.session_state.realtime_channel = channel
        # Keep thread alive (supabase realtime runs its own loop)
        while True:
            time.sleep(1)
    except Exception as e:
        st.error(f"Realtime subscription failed: {e}")
        st.session_state.realtime_initialised = False


def _ensure_realtime_thread(supabase):
    """Start background thread only once."""
    if not st.session_state.get("realtime_thread_started", False):
        thread = threading.Thread(
            target=_start_realtime_listener,
            args=(supabase,),
            daemon=True
        )
        thread.start()
...
# -------------------------------------------------------------------
# Helper: fetch full data (initial load and manual refresh)
# -------------------------------------------------------------------
@st.cache_data(ttl=5, show_spinner=False)
def _fetch_all_messages(supabase):
    """Cached fetch of all messages (cleared on realtime event)."""
    return get_all_messages(supabase)


def _refresh_data(supabase):
    """Clear cache and reload all messages."""
    st.cache_data.clear()
    st.session_state.messages_data = _fetch_all_messages(supabase)
    st.session_state.new_realtime_event = False
    st.rerun()
...
def show_admin_dashboard(supabase):
    ...
    _ensure_realtime_thread(supabase)
    ...
    if "messages_data" not in st.session_state:
        st.session_state.messages_data = _fetch_all_messages(supabase)
    if "new_realtime_event" not in st.session_state:
        st.session_state.new_realtime_event = False

    # If realtime notified a change, refresh data (only once per event)
    if st.session_state.new_realtime_event:
        # Clear the cache and reload
        st.cache_data.clear()
        st.session_state.messages_data = get_all_messages(supabase)
        st.session_state.new_realtime_event = False
        st.rerun()
    ...
    # ════════════════════════════
    # TAB 1 — Conversations (REALTIME LIVE)
    # ════════════════════════════
    with tab1:
        # Header: Refresh button + live indicator
        col1, col2 = st.columns([1, 3])
        with col1:
            if st.button("🔄 Refresh", use_container_width=True):
                _refresh_data(supabase)
        with col2:
            # Always show "LIVE" badge with pulsing animation
            st.markdown("""
            <div style="background:#052e16; border:1px solid #22c55e;
                ...
                <span style="color:#22c55e; font-weight:600; font-size:14px;">
                    LIVE — realtime updates</span>
            </div>
            <style>
            @keyframes pulse {
        ...
        data = st.session_state.messages_data

        if not data:
            _empty_state(
                "💬", "No Conversations Yet",
                "When users send messages they will appear here instantly."
            )
        else:
            ...
            with col_users:
                ...
                for usr in user_list:
                    ...
                    # Count unread replies (messages with a reply not yet seen by customer)
                    unread = sum(
                        ...
                        with st.form(key=f"reply_form_{st.session_state.selected_user_id}", clear_on_submit=True):
                            ...
                            if submitted:
                                if not reply_text.strip():
                                    ...
                                    # Find the most recent unreplied message from this user
                                    ...
                                    if unreplied:
                                        ...
                                        st.success("✅ Reply sent! It will appear instantly.")
            ...
            # ---- Metrics and chart (unchanged) ----
            st.markdown("---")
            col_m1, col_m2, col_m3 = st.columns(3)
            col_m1.metric("Total Messages", len(data))
            col_m2.metric("Total Users", len(user_list))
            col_m3.metric("Unreplied", sum(1 for m in data if not m.get('reply')))
            ...
            if not users_df.empty:
                ...
                if not daily.empty:
                    ...
                    _empty_state("📈", "No Chart Data Yet", "Message activity will appear here once users start chatting.")
            else:
                _empty_state("📈", "No Data", "No messages yet.")

    # ════════════════════════════
    # TAB 2 — User Management (unchanged)
    # ════════════════════════════
    with tab2:
        st.markdown('<div class="section-heading">User Management</div>', unsafe_allow_html=True)
        st.markdown(
        ...
        if not all_users:
            _empty_state("👥", "No Users Found", "No users have signed up yet.")
        else:
            ...
            for usr in all_users:
                ...
                with col_email:
                    ...
                    st.markdown(
                        f'<div style="color:#F0F4F8; font-size:14px; padding-top:8px;">{email}'
                        f'<span style="color:#64748B; font-size:12px;">{self_label}</span></div>',
                        unsafe_allow_html=True
                    )
                with col_badge:
                    st.markdown(
                        f'<div style="background:{badge_bg}; color:{badge_fg}; border:1px solid {badge_border}; '
                        f'border-radius:20px; padding:6px 12px; font-size:11px; font-weight:700; '
                        f'text-transform:uppercase; text-align:center; margin-top:4px;">{role}</div>',
                        unsafe_allow_html=True
                    )
                with col_action:
                    if is_self:
                        st.markdown(
                            '<div style="color:#64748B; font-size:12px; padding-top:10px; text-align:center;">Your account</div>',
                            unsafe_allow_html=True
                        )
                    else:
                        new_role = "user" if role == "admin" else "admin"
                        btn_label = "Demote to User" if role == "admin" else "Promote to Admin"
                        if st.button(btn_label, key=f"role_btn_{uid}", use_container_width=True):
                            with st.spinner("Updating role..."):
                                success, message = update_user_role(supabase, uid, new_role)
                            if success:
                                admin = st.session_state.user
                                log_action(
                                    supabase, admin["id"], admin["email"],
                                    "role_changed", f"{email}: {role} → {new_role}"
                                )
                                st.success(f"{email} is now {new_role.upper()}.")
                                st.rerun()
                            else:
                                st.error(message)
                st.markdown('<hr style="border-color:#1f2a36; margin:8px 0;">', unsafe_allow_html=True)

    # ════════════════════════════
    # TAB 3 — Audit Log (unchanged)
    # ════════════════════════════
    with tab3:
        st.markdown('<div class="section-heading">Audit Log</div>', unsafe_allow_html=True)
        with st.spinner("Loading audit log..."):
            logs = get_audit_logs(supabase, limit=100)

        if not logs:
            _empty_state("📋", "No Audit Logs Yet", "User actions will be recorded here.")
        else:
            action_colours = {
                "login": ("#dcfce7", "#166534"),
            ...
            for log in logs:
                ...
                bg, fg = action_colours.get(action, ("#1f2a36", "#94A3B8"))
                st.markdown(f"""
                <div style="background:linear-gradient(145deg,#111827,#0b1220);
                    ...
                    <div style="flex:1;">
                        <div style="color:#F0F4F8;font-size:13px;font-weight:600;">{email}</div>
                        <div style="color:#94A3B8;font-size:12px;">{details}</div>
                    </div>
                    <div style="color:#64748B;font-size:11px;white-space:nowrap;">{ts}</div>
                </div>
                """, unsafe_allow_html=True)
