...
def show_admin_dashboard(supabase):
    # Hard block — only admins can reach this page
    require_role(["admin"])
    ...
    if not data:
        ...
        ">
            ...
            <div style="color:#F0F4F8; font-size:18px; font-weight:600;
                        margin-bottom:8px;">No conversations yet</div>
    ...
    unique_users = users_df[['user_id', 'name', 'email']].drop_duplicates(
        subset=['user_id']
    )
    ...
    with col_users:
        ...
        for usr in user_list:
            user_msgs = [m for m in data if m['user_id'] == usr['user_id']]
            unread = sum(
            ...
            if st.button(label, key=f"user_{usr['user_id']}", use_container_width=True):
