...
def show_loan_analysis(model):
    ...
    with col1:
        ...
        income = st.number_input(
            "Monthly Income (KES)", 0, 1_000_000, 50_000, step=1_000, key="income"
        )
        loan_amount = st.number_input(
            "Loan Amount (KES)", 0, 1_000_000, 200_000, step=1_000, key="loan_amount"
        )
        interest_rate = st.number_input(
            "Interest Rate (%)", 0.0, 100.0, 12.5, step=0.1, key="interest_rate"
        )
        loan_term = st.selectbox(
            "Loan Term (months)", [12, 24, 36, 48, 60], key="loan_term"
        )
    with col2:
        car_value = st.number_input(
            "Car Value (KES)", 0, 1_000_000, 400_000, step=1_000, key="car_value"
        )
        car_age = st.slider("Car Age (years)", 0, 50, 5, key="car_age")
        mileage = st.number_input(
            "Mileage (km)", 0, 500_000, 80_000, step=1_000, key="mileage"
        )
        ...
        if st.button("Check Loan Risk", use_container_width=True):
            try:
                ...
                with st.spinner("Running AI risk assessment..."):
                    emp = {"salaried": 0, "self-employed": 1, "informal": 2}[
                        employment_type
                    ]
                    df = pd.DataFrame({
                        'age':               [age],
